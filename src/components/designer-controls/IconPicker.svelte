...
<!-- PIKT: disambiguate from the OpenMoji StickerPicker tile — this opens the Material-Symbols /
     user-SVG icon library, not decorative stickers. Icon + label changed accordingly (P5 final review).
     "style" chosen over "category"/"shapes" since neither exists in the offline curated icon set
     (src/styles/icon_data.ts, generated by scripts/gen-icons.mjs).
     Upstream PR candidate: no -->
<button class="tool-cell" onclick={() => (open = true)}>
  <span class="tile-chip tile-pink"><MdIcon icon="style" /></span><span>{$tr("editor.toolbar.sticker")}</span>
</button>
